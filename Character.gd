...
#	next_nav_position = self.global_position
	...
	# set the initial target location to nav_destination i.e. current location
...
func _physics_process(delta : float) -> void:
	...
	var desired_velocity = self.global_position.direction_to(next_nav_position) * character_speed_multiplier
...
func character_velocity_computed(calculated_velocity : Vector2):
