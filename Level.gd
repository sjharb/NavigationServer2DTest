...
func _draw():
	# Rectangle to show where 0,0 and 1280,720 constraints
	#draw_line(Vector2(0,0), Vector2(1280, 0), Color(0.0, 0.0, 1.0, 1.0), 3.0, false)
	#draw_line(Vector2(1280,0), Vector2(1280, 720), Color(0.0, 0.0, 1.0, 1.0), 3.0, false)
	#draw_line(Vector2(1280,720), Vector2(0, 720), Color(0.0, 0.0, 1.0, 1.0), 3.0, false)
	#draw_line(Vector2(0,720), Vector2(0, 0), Color(0.0, 0.0, 1.0, 1.0), 3.0, false)
	for character in characters:
		if character is KinematicBody2D and is_instance_valid(character) and character.is_inside_tree():
			if character.character_nav_path.size() > 1:
				...
				#if !character.character_nav_path.empty():
				for path_index in range(1, character.character_nav_path.size()):
					...
					if previous_line_point is Vector2 and line_point is Vector2 and previous_line_point.distance_to(line_point) > 1.0:
						#print("previous_line_point = ", previous_line_point)
			...
			#print("character.global_position + character.velocity = ", character.global_position + character.velocity)
